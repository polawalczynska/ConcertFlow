    ...
    public AuthResponse login(LoginRequest loginRequest) {
        try {
            ...
            User user = userRepository.findByEmail(loginRequest.email())
                .orElseThrow(() -> new UsernameNotFoundException(ErrorMessage.USER_NOT_FOUND.message()));

            if (!user.getActive()) {
                throw new DisabledException(ErrorMessage.USER_DISABLED.message());
            }

            String jwt = jwtService.generateToken(user);

            return new AuthResponse(jwt, user.getEmail(), user.getRole(), user.getId());
