...
public final class TechnicalAreaColorGenerator {
    ...
    private TechnicalAreaColorGenerator() {
        throw new UnsupportedOperationException("Utility class");
    }

    public static String generateColor(int index, int totalAreas) {
        if (totalAreas <= 0) {
            return DEFAULT_COLOR;
        }

        float hue = normalizeHue(BASE_HUE + (index * HUE_RANGE / totalAreas) - (HUE_RANGE / 2));
