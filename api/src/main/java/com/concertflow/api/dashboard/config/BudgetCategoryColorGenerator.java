...
public final class BudgetCategoryColorGenerator {
    ...
    private BudgetCategoryColorGenerator() {
        throw new UnsupportedOperationException("Utility class");
    }

    public static String generateColor(int index, int totalCategories) {
        if (totalCategories <= 0) {
            return DEFAULT_COLOR;
        }

        float hue = normalizeHue(BASE_HUE + (index * HUE_RANGE / totalCategories) - (HUE_RANGE / 2));
