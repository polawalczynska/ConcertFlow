...
interface RecentActivityProps {
  activities: RecentBudgetActivity[];
}
    ...
    return (
      <Card className="border-0 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Recently updated budgets</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="flex-1 min-w-0">
                ...
                {activity.approvedAmount && (
                  <p className="text-sm text-text-secondary mt-1">
                    ${activity.approvedAmount.toLocaleString()}
                  </p>
                )}
                <p className="text-xs text-text-secondary mt-1">Updated: {activity.lastUpdated}</p>
              </div>
              <Badge className={getStatusBadgeClasses(activity.budgetStatus)}>
                {formatStatusLabel(activity.budgetStatus)}
              </Badge>
            </div>
          ))}
        </div>
